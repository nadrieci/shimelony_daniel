...
# scp -r /home/jortega/Desktop root@10.200.21.46:/srv/shiny-server/Shimelony/www/images/plot1
  ...
  observeEvent(input$newTab, {
    ...
    appendTab("myTabs", tabPanel(paste("Plot",tabIndex()),
                                 ...
                                 output$foo <- downloadHandler(
                                   filename = function(){
                                     paste('plot',tabIndex(),'.png', sep="")},
                                   content = function(file) {
                                     device <- function(...) {
                                       grDevices::png(..., width = 750, height = 1000,
                                                      res = 300, units = "in")
                                     }
                                     # browser()
                                 ...
                                 #     #                  res = 300, units = "in")
                                 #     # }
                                 #     ggsave(filename = "test.png", plot = current_plot
                                 #            , path = paste(getwd(),"/www/images", sep=""), width = 8, height =  6)
                                 #   }),
                                 
                                 conditionalPanel(
                                   condition = paste0("input.plot_",tabIndex()," == 1"),
                                 ...
                                 conditionalPanel(
                                   ...
                                   tagList( 
                                     ...
                                     selectizeInput(
                                       ...
                                       choices = c("Stacked","Correlogram","Clustering","Diverging","Snpeff","Correlation"), selected = "Correlogram",
                                       multiple = FALSE),
###########################################################STACKED#################################                                     
                                     conditionalPanel(condition= paste0("input.type_graph_",tabIndex(),".indexOf('Stacked') != -1"),
                                     tagList(
...
#########################################################CORRELOGRAM############################################
conditionalPanel(condition= paste0("input.type_graph_",tabIndex(),".indexOf('Correlogram') != -1"),
                 tagList(
                   selectizeInput(paste("correlogram",tabIndex(), sep="_"), "Categorical Variable: ",
                                  choices = c("Genes","Tissue")
                                  ,
                                  selected = "Tissue"))
),
...
conditionalPanel(condition= paste0("input.type_graph_",tabIndex(),".indexOf('Clustering') != -1"),
                 tagList(
                   selectizeInput(paste("clustering",tabIndex(), sep="_"), "Categorical Variable: ",
                                  choices = c("Genes","Tissue")
                                  ,
                                  selected = "Tissue"),
                   checkboxInput(paste("log_clustering",tabIndex(), sep="_"), "Log")
                   )
),
#########################################################DIVERGING################################
conditionalPanel(condition= paste0("input.type_graph_",tabIndex(),".indexOf('Diverging') != -1"),
                 tagList(
                   selectizeInput(paste("diverging",tabIndex(), sep="_"), "Select one tissue ",
                                  choices = names(filtering()[,names(which(lapply(filtering(), class)=="numeric"))[c(-1,-2)]])
                                  ,
                                  selected = "Flower", multiple = TRUE))
)
                                   )#End of main tagList
...
##############################################STACKED#################################################
                                     if(input[[paste("type_graph",tabIndex(), sep="_")]]=="Stacked"){
                                     ggsave(paste("www/images/plot",tabIndex(),".png", sep=""), plot_all(filtering(),input[[paste("categorical",tabIndex(), sep="_")]],
                                                                                                           input[[paste("numerical",tabIndex(), sep="_")]], input[[paste("categorical2",tabIndex(), sep="_")]])  )
                                     current_plot <<- ggplot_3v(filtering(),input[[paste("categorical",tabIndex(), sep="_")]],
                                                               input[[paste("numerical",tabIndex(), sep="_")]], input[[paste("categorical2",tabIndex(), sep="_")]])
                                     }
############################################CORRELOGRAM###############################################                                     
                                     if(input[[paste("type_graph",tabIndex(), sep="_")]]=="Correlogram"){
                                       
                                       if(input[[paste("correlogram",tabIndex(), sep="_")]]=="Genes"){
                                       shiny::validate(
                                         need(nrow(filtering()) <= 15, "Selecting so many genes is not a good idea, believe me I am a machine")
                                       )}
                                   
                                       ggsave(paste("www/images/plot",tabIndex(),".png", sep=""), ggplot_correlogram(filtering(),input[[paste("correlogram",tabIndex(), sep="_")]]) )
                                       current_plot <<- ggplot_correlogram(filtering(),input[[paste("correlogram",tabIndex(), sep="_")]])
                                       
                                     }
############################################CLUSTERING##############################################                                     
                                     if(input[[paste("type_graph",tabIndex(), sep="_")]]=="Clustering"){
                                       
                                       if(input[[paste("clustering",tabIndex(), sep="_")]]=="Genes"){
                                         shiny::validate(
                                           need(nrow(filtering()) <= 15, "selecting so many genes is not a good idea, believe me I am a machine")
                                         )}
                                       
                                       ggsave(paste("www/images/plot",tabIndex(),".png", sep=""), ggplot_clustering(filtering(),input[[paste("clustering",tabIndex(), sep="_")]], input[[paste("log_clustering",tabIndex(), sep="_")]]))
                                       current_plot <<- ggplot_clustering(filtering(),input[[paste("clustering",tabIndex(), sep="_")]], input[[paste("log_clustering",tabIndex(), sep="_")]])
                                       
                                     }
############################################DIVERGING##############################################                                     
                                     if(input[[paste("type_graph",tabIndex(), sep="_")]]=="Diverging"){
                                       
                                         shiny::validate(
                                           need(nrow(filtering()) <= 50, "Selecting so many genes is not a good idea, believe me I am a machine")
                                         )
                                       
                                       ggsave(paste("www/images/plot",tabIndex(),".png", sep=""), ggplot_diverging_bars(filtering(),input[[paste("diverging",tabIndex(), sep="_")]]) , height = 15, width = 12, units = "in")
                                       current_plot <<- ggplot_diverging_bars(filtering(),input[[paste("diverging",tabIndex(), sep="_")]])
  ...
  output$report <- downloadHandler(
    ...
    content = function(file) {
      ...
      # browser()
      params <- list(variant = input$variant_family)
